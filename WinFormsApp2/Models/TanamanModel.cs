...
namespace WinFormsApp2.Models
{
    public class TanamanModel
    {
        ...
        {
            string query = "SELECT id_tanaman AS \"ID\", nama_tanaman AS \"Nama Tanaman\", jenis_komoditas AS \"Jenis\", varietas AS \"Varietas\", tanggal_tanam AS \"Tgl Tanam\" FROM tanaman ORDER BY id_tanaman DESC";
            return db.ExecuteQuery(query);
        }

        public void InsertTanaman()
        {
            string query = "INSERT INTO tanaman (nama_tanaman, varietas, umur_tanaman, tanggal_tanam, jenis_komoditas) VALUES (@nama, @varietas, @umur, @tanggal, @komoditas)";
            using (NpgsqlCommand cmd = new NpgsqlCommand(query))
            {
                ...
                cmd.Parameters.AddWithValue("@tanggal", TanggalTanam);
